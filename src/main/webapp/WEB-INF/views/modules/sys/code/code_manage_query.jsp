...
<html lang="en">
		...
		<div class="testPaper">
			...
			<div class="searchPanel">
				...
				<div style="margin:16px">
					...
					<span>
						<a href="javascript:;" class="commonBtn" onclick="madeKeyCode()">在线生成正式版</a>
					</span>
					<span>
						<a href="javascript:;" class="commonBtn" onclick="madeCeshiKeyCode()">在线生成测试版</a>
					</span>
				...
				<table cellpadding="0" cellspacing="0">
					...
					<tbody>
						<c:forEach items="${pageInfo.list}"  var ="codeList">
							<tr>
								<td><input type="checkbox" class="checkChoose" name="keyId" value="${codeList.keyId}" /></td>
								<td><a href="javascript:;"  filePath="${filePath}/${codeList.qrCodePath}" class="showQRCodeBtn">${codeList.keyId}</a></td>
								<td>${codeList.useNum}</td>
								<td>${codeList.schoolName}</td>
								<td><fmt:formatDate value="${codeList.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td versionName="${codeList.isCeshi}" class="versionName"></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
	...
	<div class="uploadFileWindow">
		<div class="uploadFileTitle">在线生成正式版激活码</div>
		<div class="uploadFileContent">
			<p>激活码数量：
				<input type="number" name="codeNum" id="codeNum" min="1"  class="inpt" />
			</p>
			<p>分发学校：
	...
	<div class="uploadFileWindow2">
		<div class="uploadFileTitle">在线生成测试版激活码</div>
		<div class="uploadFileContent">
			<p>激活码数量：
				<input type="number" name="codeNum2" id="codeNum2" min="1"  class="inpt" />
			</p>
			<p>分发学校：
				<select id="schoolId2" class="inpt">
					<option value="">—请选择—</option>
					<c:forEach items="${schoolList}"  var="schoolList" >
						<option value="${schoolList.schoolId}">${schoolList.schoolName}</option>
					</c:forEach>
			...
			<p>
				<a href="javascript:;" class="commonBtn" onclick="confirmMade2();">开始生成</a>
				<a href="javascript:;" class="commonBtn" onclick="closeWin2();">取消</a>	
			</p>
		</div>
	</div>
	<div class="promptBox">
...
function madeCeshiKeyCode(){
	$('.uploadFileWindow2').fadeIn();
}
function closeWin(){
	$('.uploadFileWindow').fadeOut();
}
function closeWin2(){
	$('.uploadFileWindow2').fadeOut();
}
...
function confirmMade2(){
	$('.uploadFileWindow2').hide();
	var codeNum = $("#codeNum2").val();
	var schoolId= $("#schoolId2").val();
	$.ajax({
		type:'post',
		url:'${ctx}/code/createCeshiKeyCode',
		data:'codeNum='+codeNum+"&schoolId="+schoolId,
		dataType:'json',
		success:function(str){
			if(str=="error"){
...
//显示版本名称
$('.versionName').each(function(){
	var versionName=$(this).attr('versionName');
	console.log(versionName);
	if(versionName=="0"&&versionName!=''){
		$(this).text("测试版");
	}
	else{
		$(this).text("正式版");
